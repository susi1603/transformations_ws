...
int main(int argc, char** argv) {
    ...
    try {
        ...
        robot.control([&model, &time, &initial_joint_position](const franka::RobotState& robot_state, franka::Duration period) -> franka::Torques
              {
                  ...
                  time += period.toSec();
                  if (time == 0.0) {
                      initial_joint_position = robot_state.q_d;
                  }
                  std::array<double, 7> new_joint_position = {{0, -M_PI_4, 0, -3 * M_PI_4, 0, M_PI_2, M_PI_4}};
